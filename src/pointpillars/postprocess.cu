...
void PostprocessCuda::DoPostprocessCuda4Arbe(
    float* cls_pred,
    const float* box_preds,
    float* host_box, 
    float* host_score, 
    ...
    //num_anchor_per_cls_ = 32768
    GPU_CHECK(cudaMemcpy(&host_score[0], cls_pred, num_anchor_per_cls_ * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    for (int class_idx = 0; class_idx < num_class_; ++ class_idx) {  // hardcode for class_map as {0, 12 , 34 , 5 ,67 ,89}
        ...
        for (size_t anchor_idx = 0 ; anchor_idx < num_anchor_per_cls_ ; anchor_idx++)
        {
            float score_ = 0;
            // float score_lower = 0;
            score_ =  1 / (1 + expf(-host_score[ class_idx * num_anchor_per_cls_ + anchor_idx ]));
            if (score_ > score_threshold_ && host_filtered_count[class_idx] < nms_pre_maxsize_)  // filter out boxes which threshold less than score_threshold
            {
                host_filtered_score[host_filtered_count[class_idx]] = score_;
                for (size_t dim_idx = 0 ; dim_idx < 7 ; dim_idx++) // dim_idx = {x,y,z,dx,dy,dz,yaw}
                { 
                    host_filtered_box[host_filtered_count[class_idx] * 7 + dim_idx] \
                    =  host_box[ class_idx * num_anchor_per_cls_ * num_output_box_feature_ + anchor_idx * num_output_box_feature_ + dim_idx];
