 ...
 * @names:
 * @description: Briefly describe the function of your function
 * @param {RotRect2D_t} &rect
 * @return {*}
 */
void calc_rect_contour(RotRect2D_t &rect)
{
...
RotRect2D_t gen_rectInfo(const Eigen::MatrixXd point, const double final_theta)
{
  // best angle
...
RotRect2D_t L_shape_Fit_Proc(const Eigen::MatrixXd point, const double maxAngle, const double minAngle, const double step)
{
  ...
  double theta_cur = minAngle;
  double theta_end = maxAngle;

  while (theta_cur <= theta_end)
  {
    rotate_mat << cos(theta_cur), sin(theta_cur),
        -1.0 * sin(theta_cur), cos(theta_cur);
  ...
  // 计算矩阵四条边的直线表达式
  RotRect2D_t fit_Rect = gen_rectInfo(point, final_theta);
